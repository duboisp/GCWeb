	...
	if ( caller === event.target || event.currentTarget === event.target ) {
		...
		$.ajax( fetchOpts )
			.done( function( response, status, xhr ) {
				...
				fetchData.plainResponse = response;
...
// Contains some custom fix for the data-json.....

// Like then there is 2 children <template> after appending cloned iteration

// When the cached_node is null because we need to iterate the JSO object and transform it as @id/@value


// Data JSON do support "import" when the data is:
/*
"code": {
	"@type": [ "@id", "rdf:HTML" ], "note": "Indicate to data-ajax that it need to Encode the content prior to trigger fetch",
	"@value": "samples/sample-1.html"
}
*/

// Contains a mod for data-ajax to encode in HTML the ajaxed content.


// For wb-swap -> Now it can map with the "@id"